        ...
        if origen in self.__almacenes and destino in self.__almacenes:
            ...
            if c != 0:
                ...
                if c >= cantidad:
                    ...
                    #Ahora tenemos dos opciones, puede ser que el almacen ya contenga ese tipo de producto por lo tanto solo tenemos que actualizar el stock
                    #pero en caso de que no lo tenga debemos de agregar el producto
                    c2 = almacen_destino.esta_producto(codigo)
                    producto = almacen_origen.get_producto(codigo)
                    if c2 != 0:
                        #Si el producto está en el almacen 
                        almacen_destino.actualizar_stock(codigo,cantidad)
                    else:
                        nuevo = Producto.copiar(producto)
                        nuevo.actualizar_cantidad(-c)
                        almacen_destino.agregar_producto(nuevo)
                        almacen_destino.actualizar_stock(codigo,(+cantidad))
